...
## 工厂方法：创建并配置 UI 实例
static func create(skills: Array[Dictionary], on_chosen: Callable) -> LevelUpUI:
...
func _build_ui() -> void:
	...
	# 升级标题
	var title := _make_label("— 升 级 —", 38, Color(1.0, 1.0, 1.0, 0.95))
	...
	# 副标题
	var sub := _make_label("选择一张技能牌", 18, Color(0.75, 0.80, 0.88, 0.85))
	sub.anchor_left   = 0.0
	sub.anchor_right  = 1.0
	sub.offset_top    = 132.0
	sub.offset_bottom = 168.0
	...
	hbox.offset_left   = -290.0
	hbox.offset_right  =  290.0
...
func _build_card(skill: Dictionary) -> Control:
	...
	if skill.get("stackable", false):
		...
		var stack_lbl := _make_label("当前 %d / %d 层" % [cur, mx], 14,
